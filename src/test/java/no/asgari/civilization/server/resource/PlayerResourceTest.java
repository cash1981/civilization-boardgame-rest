...
public class PlayerResourceTest extends AbstractCivilizationTest {
    ...
    public void saveSOTThenReveal() throws Exception {
        ...
        URI uri = UriBuilder.fromPath(String.format(BASE_URL + "/player/%s/turn/save", getApp().pbfId)).build();
        Response response = client().target(uri)
                .request(MediaType.APPLICATION_JSON)
                .header(HttpHeaders.AUTHORIZATION, getUsernameAndPassEncoded())
        ...
        uri = UriBuilder.fromPath(String.format(BASE_URL + "/player/%s/turn/reveal", getApp().pbfId)).build();
        response = client().target(uri)
                .request(MediaType.APPLICATION_JSON)
                .header(HttpHeaders.AUTHORIZATION, getUsernameAndPassEncoded())
                .put(Entity.json(turnDTO), Response.class);
    ...
    public void lockTurn() throws Exception {
        ...
        URI uri = UriBuilder.fromPath(String.format(BASE_URL + "/player/%s/turn/lock", getApp().pbfId)).build();
        Response response = client().target(uri)
                .request(MediaType.APPLICATION_JSON)
                .header(HttpHeaders.AUTHORIZATION, getUsernameAndPassEncoded())
                .put(Entity.json(turnDTO), Response.class);
